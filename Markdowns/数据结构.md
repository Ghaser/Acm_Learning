...
struct treeNode{
    ...
    int pid;      //数组下标索引
    int lson(){
       	return pid<<1;//左移一位代表数值*2
    }
    int rson(){
        return pid<<1|1;//再或上1代表加1
    }
}node[MAXNODES];
```
...
        nodes[p].reset(p, l, r);//初始化数据域
        if (l < r) {
            int mid = (l + r) >> 1;
            segtree_build(p<<1, l, mid);//递归构造左子树       
            segtree_build(p<<1|1, mid+1, r);//递归构造右子树    
